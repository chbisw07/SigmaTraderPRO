...
class WebhookIngestionService:
    ...
    def ingest_tradingview(
        ...
        *,
        ...
        token_ok, token_err = validate_route_token(payload)
        if not token_ok:
            msg = (
                "TradingView webhook rejected: route_token missing"
                if token_err == TradingViewWebhookReasonCode.WEBHOOK_TOKEN_MISSING
                else "TradingView webhook rejected: invalid route_token"
            )
            http_status = (
            ...
            idem = derive_idempotency_key(
                ...
                schema_version=schema_version,
                namespace=(
                    "tv_rej_token_missing"
                    if token_err == TradingViewWebhookReasonCode.WEBHOOK_TOKEN_MISSING
                    else "tv_rej_token_invalid"
                ),
            )
            ingestion = self._persist(
                db,
